...
export class UIManager {
  ...
  syncStats(stats: PlayerStats): void {
    ...
    document.getElementById('stat-power')!.textContent = stats.power.toFixed(0);
    document.getElementById('stat-size')!.textContent = stats.size.toFixed(1);
    document.getElementById('stat-multiplier')!.textContent = stats.multiplier.toFixed(1);
  ...
  renderUpgrades(save: SaveData, buy: (key: keyof PlayerStats) => void): void {
    ...
    list.innerHTML = '';
    (['speed', 'power', 'size', 'multiplier'] as (keyof PlayerStats)[]).forEach((key) => {
