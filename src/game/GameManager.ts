...
export class GameManager {
  ...
  private store = new Store<AppState>({ runState: 'landing', stats: { fireRate: 1, damage: 1, crowd: 1, income: 1 }, progress: 0 });
  ...
  private loop(time: number): void {
    ...
    if (this.running && state.runState === 'running') {
      const progress = this.scene.update(dt, this.input.targetX, state.stats, (gate) => this.applyGate(gate), () => this.hit(), (damage) => this.baseHit(damage));
  ...
  private applyGate(gate: GateConfig): void {
    ...
    this.analytics.track('gate_collect', { type: gate.kind, amount: gate.value });
  }
  ...
  private baseHit(damage: number): void {
    this.earned += Math.max(1, Math.floor(damage * 2 * this.store.get().stats.income));
    this.scene.cameraManager.punch(0.18);
    this.audio.play('coin');
    this.ui.toast('BASE HIT!', '#ffd15c');
  }
  ...
  private finish(): void {
    ...
    this.running = false;
    const stats = this.store.get().stats;
    this.earned += Math.floor((70 + this.save.data.level * 16 + stats.damage * 20) * stats.income);
  ...
  private buyUpgrade(key: keyof PlayerStats): void {
    ...
    this.save.data.upgrades[key] += key === 'crowd' ? 0.35 : 0.2;
    this.save.save();
